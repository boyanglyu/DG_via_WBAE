...
def save_checkpoint(args, hparams, rank, filename, dataset, algorithm):
    if rank == 0:
        save_dict = {
            "args": vars(args),
    ...
    print('file name is ', os.path.join(args.output_dir, filename))
    dist.barrier()
...
def main(rank, world_size):
    ...
    eval_loaders = [FastDataLoader(
        dataset=env,
        batch_size=64,
        num_workers=dataset.N_WORKERS,rank=rank,
        world_size=world_size,
        shuffle=True)
    ...
    algorithm = algorithm_class(rank, dataset.input_shape, dataset.num_classes,
    ...
    def set_epoch_for_loaders(train_loaders, steps):
    ...
    for step in range(start_step, n_steps):
        ...
        train_minibatches_iterator = set_epoch_for_loaders(train_loaders, step)
        minibatches_device = [(x.to(rank), y.to(rank))
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            #     torch.load(os.path.join(args.output_dir, f'model_step{step}.pkl'), map_location=map_location)["model_dict"]
            # )
