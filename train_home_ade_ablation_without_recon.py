...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 18 18:48:11 2021

@author: boyanglyu
"""

'''
Use ResNet 50 feature for but don't use test data for barycenter, change the
structure for classifier, use only one linear layer for classifier
'''
import argparse
...
#repeat 1 is increase baryNum 100 (org is batch size)
    ...
    for alpha in alpha_list:
        for beta in beta_list:
            for test_file in test_file_list:
                ...
                if not os.path.exists('office_home_new_val/without_recon/home_checkpoint_AED' + str(repeat)):
                    os.makedirs('office_home_new_val/without_recon/home_checkpoint_AED'+ str(repeat))
                if not os.path.exists('office_home_new_val/without_recon/history_home_AED'+ str(repeat)):
                    os.makedirs('office_home_new_val/without_recon/history_home_AED'+ str(repeat))

                check_folder = 'office_home_new_val/without_recon/home_checkpoint_AED'+ str(repeat) + '/'
                history_folder = 'office_home_new_val/without_recon/history_home_AED'+ str(repeat) + '/'
                ...
                optimizer_ae = optim.Adam((list(ae_model.parameters()) + list(clf_model.parameters())), lr=learning_rate)

                AE_model, Clf_model, history_dict = train_model(ae_model, clf_model, generator, val_data,
