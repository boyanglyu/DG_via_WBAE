...
def train_model(ae_model,clf_model, d_model, generator,
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 1 == 0:
            ...
            print('Epoch ', epoch)
            print('Training Loss: {:f}, Clf loss: {:f}, Recon loss: {:f},'.format(total_loss, train_clf_loss, train_recon_loss))
    ...
    for alpha in alpha_list:
        for beta in beta_list:
            for test_file in test_file_list:
                ...
                if not os.path.exists('vlcs_new_val/without_bary/vlcs_checkpoint_AED' + str(repeat)):
                    os.makedirs('vlcs_new_val/without_bary/vlcs_checkpoint_AED'+ str(repeat))
                if not os.path.exists('vlcs_new_val/without_bary/history_vlcs_AED'+ str(repeat)):
                    os.makedirs('vlcs_new_val/without_bary/history_vlcs_AED'+ str(repeat))

                check_folder = 'vlcs_new_val/without_bary/vlcs_checkpoint_AED'+ str(repeat) + '/'
                history_folder = 'vlcs_new_val/without_bary/history_vlcs_AED'+ str(repeat) + '/'
