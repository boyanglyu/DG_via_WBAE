...
def get_result(check_path, test_data, batch_size, run_time):
    ...
    with open(history_folder + test_file + '_results_' + 'lr' + str(learning_rate) + 'bs' + str(batch_size)+'epoch' + str(num_epochs) + 'alpha' + str(alpha) + 'beta' + str(beta)+'itr'+str(iteration)+'drop'+str(dropout)+'blur'+str(blur)+ '.txt', 'a+') as f:
        f.write('\n')
    ...
    for alpha in alpha_list:
        for beta in beta_list:
            for test_file in test_file_list:
                ...
                if not os.path.exists('vlcs_new_val/vlcs_checkpoint_AED' + str(repeat)):
                    os.makedirs('vlcs_new_val/vlcs_checkpoint_AED'+ str(repeat))
                if not os.path.exists('vlcs_new_val/history_vlcs_AED'+ str(repeat)):
                    os.makedirs('vlcs_new_val/history_vlcs_AED'+ str(repeat))

                check_folder = 'vlcs_new_val/vlcs_checkpoint_AED'+ str(repeat) + '/'
                history_folder = 'vlcs_new_val/history_vlcs_AED'+ str(repeat) + '/'
